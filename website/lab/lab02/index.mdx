...
To print messages to serial, use these functions.
...
{
    UCSR0B = 0; // Disable interrupts, TX, RX
    UCSR0A = 0; // Clear flags
    UCSR0C = 0; // Reset mode (async, 8N1)
    UBRR0H = 0; // Clear baud rate high byte
    UBRR0L = 0; // Clear baud rate low byte
